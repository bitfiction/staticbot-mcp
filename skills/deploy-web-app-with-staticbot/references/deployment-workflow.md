...
1. Call `get_account_status`, follow its `pendingAction`, then call `list_source_repositories` with no arguments — one call covers every connected GitHub and GitLab account — and resolve the current repository from client/project context. Private repositories are supported. Use one unambiguous match without asking; when several match, present them with their `sourceLabel` (the account each is hosted in). A `sources[]` entry with an `unavailableReason` is a missing account, not a missing repository. If there are no sources, direct the user to `https://app.staticbot.dev/integrations`, then retry. Never claim a public repository is required.
2. Call `list_templates`; use `get_template` for a plausible existing template or `create_template` for the resolved repository, passing its `integrationInstanceId` as `sourceControlIntegrationInstanceId` so the scan uses the account that can read it.
