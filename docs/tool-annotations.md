...
- **`openWorldHint: true`** — the call changes state the public can see: what a website serves, or
  records in a live DNS zone. Reading a public URL (`scan_deployed_url`) does not count; the hint is
  about writing.
...
- `complete_migration_job` records that a manual step the user performed is done.
- `create_deployment` prepares a deployment; `start_deployment` is what publishes.

## Justifications for the destructive seven
