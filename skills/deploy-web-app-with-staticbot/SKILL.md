...
1. Call `get_account_status` first and follow its `pendingAction`: stop on `CONNECT_SOURCE_CONTROL`, and on `CHOOSE_PATH` confirm the user wants hosting rather than a migration (hosting leaves the app's backend untouched). Then resolve the repository before asking the user for its URL: call `list_source_repositories` with no arguments — it covers every connected GitHub and GitLab account. Private repositories are supported. Use an unambiguous match from the current client/project context; when several match, show each one's `sourceLabel` (the account it is hosted in) and ask. If nothing is connected, direct the user to `https://app.staticbot.dev/integrations`, then retry.
2. Reuse an existing template only when it represents the resolved repository and revision; otherwise call `create_template` so Staticbot scans the repo, passing the repository's own `integrationInstanceId` as `sourceControlIntegrationInstanceId`.
