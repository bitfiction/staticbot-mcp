...
- `POST /cloudflare/hosting-preflight`

Where a Cloudflare Workers / SSR app is hosted. `hosting-targets` returns the choices: Staticbot's
managed account (`managed`) plus each connected customer account (`customerOptions`), with
`notices` explaining any account that could not be offered. Render the `label`s; pass the `value`s
back **verbatim** — they are opaque. `customerOptionsAvailable: false` means there is no choice to
put to the user. Supplying `stackId` adds the stack's current choice and hostname as
`selectedValue` / `selectedHostname`.

`hosting-preflight` (body: `choice`, `hostname`) validates a choice before anything is created:
it resolves the Cloudflare zone, checks the account can host Workers, and reports collisions.
It is read-only and answers for the managed choice too, so call it the same way either way. When
`ok` is false, show `blockingMessage` to the user and create nothing; `warnings` are non-blocking.
A customer `hostname` must already live in one of that account's zones (apex is supported, an
autogenerated Staticbot subdomain is not).

Pass the choice to `POST /stacks` or `POST /deployments` as `cloudflareChoice` (plus
`cloudflareHostname` for a deployment). Omit it for Staticbot hosting. Ownership follows from the
choice — do not send an account ID, a zone ID, or `infrastructureOwnership` for Cloudflare.
